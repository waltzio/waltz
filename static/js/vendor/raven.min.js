...
function send(data) {
    ...
    if (isFunction(globalOptions.shouldSendCallback)) {
        var shouldSend = globalOptions.shouldSendCallback(data);
        if (!shouldSend) {
            return;
        } else if (isFunction(shouldSend.resolve)) {
            // it's promise, woot
            $.when(shouldSend).then(function(_shouldSend) {
                if (_shouldSend) _send();
            });
        } else {
            _send();
        }
...
Raven.config(
    'https://44c9ca2e869c4e0c9c9c699944467e04@app.getsentry.com/25602',
    {
        shouldSendCallback: function() {
            var promise = $.Deferred();
            $.when(Utils.isDevMode()).then(function(isDevMode) {
                promise.resolve(!isDevMode);
            });
            return promise;
        }
    }
